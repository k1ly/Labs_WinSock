...
bool GetRequestFromClient(char* name, short port, struct sockaddr* from, int* flen)
{
	...
	while (true)
	{
		...
		if (strcmp(ibuf, name) == 0) {
			hostent* addr = gethostbyaddr(((char*)&clnt.sin_addr), sizeof(clnt.sin_addr), AF_INET);
			if (addr == NULL)
				throw SetErrorMsgText("gethostbyaddr:", WSAGetLastError());
			cout << "Client IP - " << inet_ntoa(clnt.sin_addr) << " (" << addr->h_name << ")" << endl;
			*from = *(sockaddr*)&clnt;
			*flen = lc;
			rc = true;
			break;
